...
| **FE-1 restantes** (`posts`, `contents`, `destaques`, `alertRules`, `internalMails`, `automations`, `integrations`, `permissions`, `scheduledJobs`) | **Mock** — `seed.*` + estado React | Domain hooks FE-4: `use-communication-store`, `use-repository-store` (só `contents`), `use-admin-store`; `// MOCK` nos slices; rotas ocultas em prod via `mock-module-gates.ts`. Migração Java: [FE-5](docs/fe-5-mock-to-java-migration.md) |
